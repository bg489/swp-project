...
export default function HealthCheckFormPage() {
  ...
  const DEFAULT_SHELF_LIFE_DAYS = 5; // Typical for platelets, but you requested +35 after donation; using 35 to match request
  const REQUESTED_SHELF_LIFE_DAYS = 35;
  ...
  useEffect(() => {
    async function fetch() {
      try {
        const response = await api.get(`/whole-blood/platelet/${bloodUnitId}`);
        ...
        const computedExpiry = expiryDateStr || formatYMD(addDays(new Date(collectionDateStr), REQUESTED_SHELF_LIFE_DAYS));
        setForm({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name === "collectionDate") {
      ...
      const exp = addDays(col, REQUESTED_SHELF_LIFE_DAYS);
  ...
  async function saveField(): Promise<void> {
    try {
      await api.put(`/whole-blood/platelet/${bloodUnitId}`, {
  ...
  volumeOrWeight: 250,
      ...
      if (inputDate <= today) {
        await api.put(`/whole-blood/platelet/${bloodUnitId}/expire`);
